...
function postDiscord(webhook, payload) {
  if (!webhook) return Promise.resolve(null);
  return fetch(webhook, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }).then(res => res.text().then(text => ({ ok: res.ok, status: res.status, body: text }))).catch(e => ({ ok: false, error: e.message }));
}
