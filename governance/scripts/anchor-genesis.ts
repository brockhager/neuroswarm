...
function logAnchoring(txSignature: string, memoContent: string, fingerprints: any): void {
  ...
  const fallbackLogPath = path.join(root, govDir, 'wp_publish_log.jsonl');
  const timelinePath = path.join(root, govDir, 'governance-timeline.jsonl');
...
function main() {
  try {
    ...
    const fingerprintPath = process.env.GENESIS_FINGERPRINTS_PATH
      ? path.resolve(process.env.GENESIS_FINGERPRINTS_PATH)
      : path.resolve(process.cwd(), '..', 'docs', 'governance', 'genesis-fingerprints.json');
