#!/bin/bash
# Relocated WP publisher setup script — admin-node/scripts
echo "🚀 Setting up NeuroSwarm WordPress Publisher (admin-node/scripts)"
echo "==========================================="
...
if [ -f ".wp_publisher.env" ]; then
    ...
    if [ -n "$WP_USERNAME" ] && [ -n "$WP_APP_PASSWORD" ]; then
        python3 admin-node/scripts/test_connection.py --username "$WP_USERNAME" --password "$WP_APP_PASSWORD" --url "${WP_SITE_URL:-https://getblockchain.tech/neuroswarm/}"
        connection_test=$?
    else
...
echo "🎉 Setup complete!"
echo ""
echo "Available commands (admin-node):"
echo "  python3 admin-node/scripts/wp_publisher.py --help          # Single file publishing"
echo "  python3 admin-node/scripts/batch_publish.py --help         # Batch publishing"
echo "  python3 admin-node/scripts/content_sync.py --help          # Auto-sync with watch mode"
echo "  python3 admin-node/scripts/test_connection.py --help       # Test WordPress connection"
echo ""
echo "Example usage:"
echo "  python3 admin-node/scripts/wp_publisher.py --content example-content.json"
echo ""
