...
export class DiscordService {
  ...
  private loadChannelConfiguration() {
    ...
    this.channels.set('genesis-anchors', process.env.DISCORD_CHANNEL_GENESIS_ANCHORS || '');
    this.channels.set('verification-results', process.env.DISCORD_CHANNEL_VERIFICATION_RESULTS || '');
    this.channels.set('governance-logs', process.env.DISCORD_CHANNEL_GOVERNANCE_LOGS || '');
    this.channels.set('alerts-critical', process.env.DISCORD_CHANNEL_ALERTS_CRITICAL || '');
    this.channels.set('alerts-info', process.env.DISCORD_CHANNEL_ALERTS_INFO || '');
    this.channels.set('system-health', process.env.DISCORD_CHANNEL_SYSTEM_HEALTH || '');
    this.channels.set('onboarding', process.env.DISCORD_CHANNEL_ONBOARDING || '');
    this.channels.set('faq', process.env.DISCORD_CHANNEL_FAQ || '');
    this.channels.set('discussion', process.env.DISCORD_CHANNEL_DISCUSSION || '');
    this.channels.set('timeline-feed', process.env.DISCORD_CHANNEL_TIMELINE_FEED || '');
    this.channels.set('audit-trail', process.env.DISCORD_CHANNEL_AUDIT_TRAIL || '');
  }
  ...
  private async sendMessage(channelKey: string, content: string, embed?: EmbedBuilder) {
    if (!this.isReady) {
      logger.warn(`Discord bot not ready, skipping message to ${channelKey}`);
      return;
    }

    const channelId = this.channels.get(channelKey);
    if (!channelId) {
      logger.warn(`No channel configured for ${channelKey}`);
      return;
    }
    ...
    try {
      ...
      if (!channel) {
        logger.error(`Could not find channel ${channelId} for ${channelKey}`);
        return;
      }
