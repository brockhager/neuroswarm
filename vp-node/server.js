...
export async function produceLoop() {
  try {
    ...
    // CN-06 safety: ensure VP sync state and apply queueing logic for REVIEW requests.
    // Check NS sync state and update our internal state machine.
    try {
      ...
      if (sync && sync.synced) {
        ...
        // Not synced — ensure we're in SYNCING_LEDGER state
        vpState.setState(STATES.SYNCING_LEDGER);
      }
    } catch (e) {
      // if check fails, mark as SYNCING to be safe
      vpState.setState(STATES.SYNCING_LEDGER);
    }
    ...
    if (critiqueProcessor) {
      for (const tx of txs.slice()) { // iterate over a copy because we may append
        if (tx.type === 'REQUEST_REVIEW') {
          // If VP is not in a safe state to process, enqueue the request and skip processing.
      ...
      if (vpState.getState() === STATES.LISTENING_FOR_REVIEWS && reviewQueue.size() > 0) {
        ...
        for (const tx of items) {
          try {
            ...
            logVp('[CN-06] Failed to generate critique from queued request', e.message);
          }
        }
      }
    }
    ...
    // safe to switch into producing state
    vpState.setState(STATES.PRODUCING_BLOCK);
    ...
    try {
      const check = await verifyNsSynced({ nsUrl: NS_URL });
      if (check && check.synced) vpState.setState(STATES.LISTENING_FOR_REVIEWS);
      else vpState.setState(STATES.SYNCING_LEDGER);
    } catch (e) {
      vpState.setState(STATES.SYNCING_LEDGER);
    }
...
async function main() {
  ...
  // Start with syncing state — we'll attempt an initial sync check and then start the produce loop
  vpState.setState(STATES.SYNCING_LEDGER);
  try {
    const initial = await verifyNsSynced({ nsUrl: NS_URL });
    if (initial && initial.synced) vpState.setState(STATES.LISTENING_FOR_REVIEWS);
  } catch (e) {
    vpState.setState(STATES.SYNCING_LEDGER);
  }
