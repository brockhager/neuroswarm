...
2. Phase 3: Key Management Integration (Vault & Registry prototype).
	- Status: **Phase 3 implemented (prototype)** — `KmsVaultClient` and `PublicKeyRegistry` classes added at `shared/key-management.ts`, integrated into VP/NS node flows. These are prototype interfaces for architecting KMS/HSM and registry integration.
