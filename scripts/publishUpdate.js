...
/*
  publishUpdate.js
  Clean ESM implementation  appends an entry to the wiki Updates.md and optionally posts to Discord and pushes the change.
*/
import fs from 'fs';
...
function usageAndExit() {
  console.error('Usage: node scripts/publishUpdate.js --title "Title" --body "Body text" [--author "Author"] [--push]');
  process.exit(1);
}
...
async function main() {
  ...
  // Resolve path relative to this script (neuroswarm/scripts)
  const __filename = fileURLToPath(import.meta.url);
  const __dirname = path.dirname(__filename);
  const updatesFile = path.resolve(__dirname, '..', 'wiki', 'Updates.md');
  ...
  if (opts.push) {
    try {
      child_process.execSync('git add -- ' + JSON.stringify(path.relative(process.cwd(), updatesFile)), { stdio: 'inherit' });
