...
for /L %%i in (1,1,60) do (
    echo [PG-WAIT] check %%i at %time% >> "%START_LOG%"
    ...
    if not errorlevel 1 (
        echo [PG-WAIT] Postgres is healthy at %date% %time% (iteration %%i) >> "%START_LOG%"
...
echo [PG-WAIT] Postgres did not report healthy after 60 checks >> "%START_LOG%"
echo Postgres did not report healthy after 60 checks
goto :eof

:healthy
echo [PG-WAIT] Completed at %date% %time% >> "%START_LOG%"
