...
const nodes = [
  { name: 'ns-node', entry: path.join('neuroswarm', 'ns-node', 'server.js'), port: 3000 },
  { name: 'gateway-node', entry: path.join('neuroswarm', 'gateway-node', 'server.js'), port: 8080 },
  { name: 'vp-node', entry: path.join('neuroswarm', 'vp-node', 'server.js'), port: 4000 }
];

const targets = [
  { os: 'linux', target: 'node20-linux-x64' },
  { os: 'macos', target: 'node20-macos-x64' },
  { os: 'win', target: 'node20-win-x64' }
];
...
for (const node of nodes) {
  for (const t of targets.filter(tt => !filter || tt.os === filter)) {
    ...
    if (res.status !== 0) {
      console.error('pkg failed for', node.name, t.os);
